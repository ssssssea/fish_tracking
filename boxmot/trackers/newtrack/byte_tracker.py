...
class BYTETracker(object):
    def __init__(
        self, track_thresh=0.45, match_thresh=0.8, track_buffer=25, frame_rate=30 #track_thresh=0.45, track_buffer = 25
    ):
    ...
    def update(self, dets, _):
        ...
        #이부분에서 predict? multi_predict 수정해야할듯
        STrack.multi_predict(strack_pool)
        dists = iou_distance(strack_pool, detections) #detection , predict 거리  계 산 (iou)      
        # if not self.args.mot20:
        dists = fuse_score(dists, detections) #matching function 안에  있음       
        ...
        for itracked, idet in matches:
            ...
            det = detections[idet]
            if track.state == TrackState.Tracked: #state = new, tracked, lost, removed
        ...
        if len(dets_second) > 0: #dets_second = thresh & lowthresh 사이 값을 가지는 detection box
            """Detections"""
        ...
        matches, u_track, u_detection_second = linear_assignment(dists, thresh=0.5) #default 0.5
        for itracked, idet in matches:
        ...
        for it in u_track:
            for idet in u_detection_second:
        ...
        dists = iou_distance(unconfirmed, detections) #unconfirmed = not activate
        # if not self.args.mot20:
